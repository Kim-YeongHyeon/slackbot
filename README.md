...
- **일일 리마인더** — 평일 **09:00 KST**, **현재 활성 스프린트**의 미해결 이슈만.
...
- 설정: `reminder.cron`(일일), `reminder.biweekly-cron`(격주 점화), `reminder.biweekly-anchor`(격주 기준 월요일), `reminder.zone`, `reminder.enabled`(전역 차단).
